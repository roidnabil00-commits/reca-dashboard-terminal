    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="px-6 py-6 max-w-6xl mx-auto">
        {activeTab === 'video' ? (
          ...
          // =========================================================
          // TAB 2: BOOKS WORKSPACE (UPGRADED INLINE PREVIEW WORKSPACE)
          // =========================================================
          bookModules.length === 0 ? (
            ...
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              {/* Left Side: Document Summary & Preview Reader Workspace */}
              <div className="flex-1">
                {activeModule && activeModule.type === 'book' && (
                  <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex flex-col justify-between min-h-[450px]">
                    <div>
                      {/* Sub-label Document */}
                      <div className="flex items-center gap-1.5 text-xs text-amber-600 font-medium font-mono uppercase mb-3">
                        <svg className="w-4 h-4 text-amber-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 0 0 6 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 0 1 6 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 0 1 6-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0 0 18 18a8.967 8.967 0 0 0-6 2.292m0-14.25v14.25" />
                        </svg>
                        Intelligence Document Abstract
                      </div>

                      <h2 className="font-display font-bold text-navy-900 text-xl border-b border-gray-100 pb-3 mb-4">{activeModule.title}</h2>
                      
                      {/* INLINE SCROLLABLE ABSTRACT PANEL */}
                      <div className="bg-gray-50 border border-gray-200 rounded-lg p-5 text-sm text-gray-700 leading-relaxed max-h-[320px] overflow-y-auto shadow-inner whitespace-pre-wrap font-light">
                    ...
                    <div className="mt-6 pt-4 border-t border-gray-100 flex items-center justify-between">
                      <span className="text-[11px] text-gray-400 font-mono uppercase tracking-wider">Vault ID: RECA-LIT-{activeModule.id}</span>
                      <a
                        href={activeModule.source_link}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 bg-navy-900 hover:bg-navy-800 text-white text-xs font-semibold px-5 py-2.5 rounded-lg transition-colors shadow-sm group"
                      >
                        Open Original Document Source
              ...
              {/* Right Side: Document Catalog Index Shelf Selector */}
              <div className="lg:w-72 xl:w-80">
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Document Index</h3>
                <div className="space-y-2">
                  {bookModules.map((m, idx) => (
                    <button
                      key={m.id}
                      onClick={() => setActiveModule(m)}
                      ...
                      <div className="flex items-center gap-3 justify-between">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className={`w-5 h-5 rounded-md flex items-center justify-center text-[10px] font-mono font-bold flex-shrink-0 ${
                            activeModule?.id === m.id ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500'
                          }`}>
                            {String(idx + 1).padStart(2, '0')}
                          </div>
                          <p className={`text-sm font-semibold leading-tight line-clamp-1 ${activeModule?.id === m.id ? 'text-white' : 'text-navy-900'}`}>
                            {m.title}
                          </p>
                        </div>
                        <svg className={`w-3.5 h-3.5 flex-shrink-0 ${activeModule?.id === m.id ? 'text-white' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
